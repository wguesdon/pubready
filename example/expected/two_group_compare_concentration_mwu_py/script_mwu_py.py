...
res = pg.mwu(a, b); p = float(res["p_val"].iloc[0])

fig, ax = plt.subplots(figsize=(3.8, 4.0))
sns.boxplot(data=df, x="group", y="concentration", hue="group", order=order, palette=['#3B6DB3', '#C1432B'], ax=ax, legend=False, width=0.6, fliersize=0)
sns.stripplot(data=df, x="group", y="concentration", hue="group", order=order, palette=['#3B6DB3', '#C1432B'],
