...
at <- as.data.frame(rstatix::anova_test(df, response ~ `genotype` * `treatment`, type = 2))
...
p <- ggplot(df, aes(x = .data[["genotype"]], y = .data[["response"]], fill = .data[["treatment"]])) +
  ...
  scale_fill_manual(values = c("#3B6DB3", "#C1432B", "#2E8B57")) +
...
ggsave("figure_response_twoway.pdf", p, width = 5.2, height = 4.4)
